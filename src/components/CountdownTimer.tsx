...
export function CountdownTimer({ targetDateISO, challengeStatus }: CountdownTimerProps) {
  const formattedTargetDate = new Date(targetDateISO).toLocaleString('de-DE', {
    weekday: 'short',
    day: '2-digit',
    month: '2-digit',
    year: 'numeric',
  ...
  if (challengeStatus === 'live') {
    return (
        ...
        <h3 className="text-2xl font-semibold text-destructive dark:text-red-400">Challenge ist LIVE!</h3>
        <p className="text-muted-foreground mt-1">Viel Erfolg – die Session läuft bereits.</p>
      </div>
    );
  }
  ...
  if (!timeLeft) { // Countdown finished, but not yet live (or status unknown)
    return (
      <div className="text-center py-10 bg-blue-50 dark:bg-blue-900/30 border border-primary/50 rounded-lg shadow-md">
        ...
        <h3 className="text-2xl font-semibold text-primary dark:text-blue-400">Startet in Kürze</h3>
        <p className="text-muted-foreground mt-1">Geplante Startzeit: {formattedTargetDate} Uhr</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gradient-to-br from-primary/5 via-card to-accent/5 dark:from-primary/10 dark:via-card dark:to-accent/10 p-6 rounded-lg shadow-inner border border-border/50">
      <p className="mb-4 text-center text-sm text-muted-foreground">
        Geplanter Start: <span className="font-semibold text-foreground">{formattedTargetDate} Uhr</span>
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-center">
        {Object.entries(timeLeft).map(([unit, value]) => {
          ...
          <div key={unit} className="bg-card/70 dark:bg-card/90 p-3 sm:p-4 rounded-lg shadow-md border border-border/50">
            ...
            <div className="text-xs sm:text-sm uppercase tracking-wider text-muted-foreground mt-0.5">{unitLabel[unit] ?? unit}</div>
          </div>
        )})}
      </div>
    </div>
